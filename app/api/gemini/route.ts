  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({
        text: jsonMode ? '{}' : "AI yordamchi hozircha sozlanmagan. LUXECORE mahsulotlari bo'yicha savollaringizni operatorga yuborishingiz mumkin.",
        audioBase64: null,
        disabled: true,
      });
    }
    ...
    // 2. TTS Generation (opt-in only — clients must pass enableTts: true)
    let audioBase64 = null;
    if (enableTts && !jsonMode) {
      try {
