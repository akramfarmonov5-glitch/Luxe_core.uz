...
export async function generateMetadata({ params }: { params: Promise<{ id: string, lang: string }> }) {
  try {
    ...
    const fallbackProduct = MOCK_PRODUCTS.find((item) => item.id === Number(productId));
    const metadataProduct = product || fallbackProduct;
    if (!metadataProduct) return { title: 'Mahsulot topilmadi' };
    ...
    return {
      ...
      openGraph: {
        title: title,
        description: productDesc,
        images: metadataProduct.image ? [{ url: metadataProduct.image }] : [],
        type: 'website'
      }
    };
  } catch (error) {
