...
const AIChatAssistant: React.FC<AIChatAssistantProps> = ({ products }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: "Assalomu alaykum! Men LUXECORE onlayn do'koni yordamchisiman. Sizga qanday yordam bera olaman? Masalan, 'premium soatlar bormi' yoki 'sumkalar narxini ayt' deb so'rashingiz mumkin." }
  ]);
  ...
  useEffect(() => {
    const savedUser = (typeof window !== 'undefined' ? localStorage.getItem('luxecore_chat_user') : null);
    if (savedUser) {
  ...
  const getSystemInstruction = () => {
    ...
    return `
      Siz LUXECORE onlayn do'konining sotuvchi-konsultantisiz.
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: data.text || "AI yordamchi hozircha sozlanmagan. Operator tez orada siz bilan bog'lanadi." }]);
          ...
          >
            <div className="p-4 border-b border-white/10 flex items-center justify-between bg-gradient-to-r from-gold-600/10 to-transparent shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-white font-bold text-sm">LUXECORE Assistant</h3>
